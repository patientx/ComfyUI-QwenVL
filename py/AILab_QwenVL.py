...
# ROCm/RDNA2 SageAttention support (triton kernel from sageattention-autotune)
try:
    from sageattention.core import sageattn_qk_int8_pv_fp16_triton
    SAGE_ATTENTION_TRITON_AVAILABLE = True
except ImportError:
    SAGE_ATTENTION_TRITON_AVAILABLE = False


def is_rocm():
    """True when torch's CUDA build is actually a ROCm/HIP build (AMD GPU)."""
...
def sage_attn_available():
    ...
    if not torch.cuda.is_available():
        return False
    if is_rocm():
        return SAGE_ATTENTION_TRITON_AVAILABLE
...
def get_sage_attention_config():
    ...
    if is_rocm():
        # torch.cuda.get_device_capability() isn't meaningful under ROCm - skip the
        # CUDA SM-version branching below entirely and use the RDNA2 triton kernel.
        print("[QwenVL] SageAttention: Using ROCm/RDNA2 Triton kernel (sageattn_qk_int8_pv_fp16_triton)")
        return sageattn_qk_int8_pv_fp16_triton, "per_warp", "fp32"
    ...
    def make_sage_forward(AttentionClass, apply_rotary_pos_emb_func):
        def sage_attention_forward(
            ...
            **kwargs,
            ...
            # Different SageAttention kernels accept different kwargs (e.g. the
            # triton backend has no qk_quant_gran param) - only pass what this
            # particular kernel function actually declares.
            sage_call_kwargs = {
                "tensor_layout": "HND",
                "is_causal": is_causal,
            ...
            try:
                accepted_params = set(inspect.signature(SAGE_ATTN_FUNC).parameters)
                sage_call_kwargs = {k: v for k, v in sage_call_kwargs.items() if k in accepted_params}
            except (TypeError, ValueError):
                pass  # signature couldn't be introspected (e.g. C extension) - pass everything as before
...
class QwenVLBase:
    ...
    def run(self, model_name, quantization, preset_prompt, custom_prompt, image, video, frame_count, max_tokens, temperature, top_p, num_beams, repetition_penalty, seed, keep_model_loaded, attention_mode, use_torch_compile, device, save_quantized_copy=False, video_frame_size="auto"):
...
class AILab_QwenVL(QwenVLBase):
    ...
    def process(self, model_name, quantization, preset_prompt, custom_prompt, attention_mode, max_tokens, keep_model_loaded, seed, save_quantized_copy=False, image=None, video=None):
        return self.run(model_name, quantization, preset_prompt, custom_prompt, image, video, 16, max_tokens, 0.6, 0.9, 1, 1.2, seed, keep_model_loaded, attention_mode, False, "auto", save_quantized_copy, video_frame_size="auto")
...
class AILab_QwenVL_Advanced(QwenVLBase):
    ...
    def process(self, model_name, quantization, attention_mode, use_torch_compile, device, preset_prompt, custom_prompt, max_tokens, temperature, top_p, num_beams, repetition_penalty, frame_count, video_frame_size, keep_model_loaded, seed, save_quantized_copy=False, image=None, video=None):
        return self.run(model_name, quantization, preset_prompt, custom_prompt, image, video, frame_count, max_tokens, temperature, top_p, num_beams, repetition_penalty, seed, keep_model_loaded, attention_mode, use_torch_compile, device, save_quantized_copy, video_frame_size=video_frame_size)
